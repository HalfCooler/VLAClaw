...
class ScrcpyFrameSource:
    ...
    def start(self) -> None:
        ...
        self._listener = threading.Thread(
            ...
            name="vlaclaw-scrcpy-frame-listener",
            daemon=True,
        )
...
class AdbBackend:
    ...
    Observations use adb screencap by default; scrcpy remains optional.
    ...
    async def _run_app_context_launcher(self, action: Action, *, timeout: float) -> str:
        ...
        remote_args = [
            ...
            launcher_component,
            "--es",
            "vlaclaw.intent_action",
            intent_action,
        ]
        if action.text:
            remote_args.extend(["--es", "vlaclaw.data_uri", str(action.text)])
        if action.package:
            remote_args.extend(["--es", "vlaclaw.package", str(action.package)])
        if action.component:
            remote_args.extend(["--es", "vlaclaw.component", str(action.component)])
        if action.mime_type:
            remote_args.extend(["--es", "vlaclaw.mime_type", str(action.mime_type)])
        if action.categories:
            remote_args.extend(["--esa", "vlaclaw.categories", ",".join(action.categories)])
        if action.extras:
            ...
            remote_args.extend(["--es", "vlaclaw.extras_json", extras_payload])
    ...
    def _get_packaged_yadb_path(self) -> Path:
        return Path(importlib.resources.files("vlaclaw").joinpath("assets/android/yadb"))

    def _write_local_temp_text(self, text: str) -> Path:
        fd, path = tempfile.mkstemp(prefix="vlaclaw-yadb-", suffix=".txt")
        with os.fdopen(fd, "w", encoding="utf-8", newline="") as handle:
            handle.write(text)
        return Path(path)

    def _make_yadb_device_text_path(self) -> str:
        return f"/data/local/tmp/vlaclaw-yadb-{uuid.uuid4().hex}.txt"

    def _write_local_temp_yadb_script(self) -> Path:
        fd, path = tempfile.mkstemp(prefix="vlaclaw-yadb-", suffix=".sh")
        script = (
            "#!/system/bin/sh\n"
            'text="$(cat "$1")"\n'
    ...
    def _make_yadb_device_script_path(self) -> str:
        return f"/data/local/tmp/vlaclaw-yadb-{uuid.uuid4().hex}.sh"
